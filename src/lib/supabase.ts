...
export async function getUserDisplayName(userId: string): Promise<string> {
  ...
  try {
    // Try to get from user_data (where we store app_state with profile.name)
    const { data } = await supabase
      .from('user_data').select('app_state')
      .or(`user_id.eq.${userId},clerk_user_id.eq.${userId}`)
      .limit(1)
      .single()
    if (data?.app_state?.profile?.name) {
      userNameCache[userId] = data.app_state.profile.name
      return data.app_state.profile.name
    }
  } catch {}
  const short = userId.substring(0, 6)
  userNameCache[userId] = 'User ' + short
  return 'User ' + short
}

// ─── SHARED LIST ACTIVITY LOG ────────────────────────────────────
...
export async function logActivity(householdId: string, userId: string, action: string, itemText?: string): Promise<void> {
  try {
...
export async function getActivityLog(householdId: string, limit: number = 30): Promise<ActivityEntry[]> {
  try {
    const { data } = await supabase
...
export async function getHouseholdMembers(householdId: string): Promise<HouseholdMember[]> {
  try {
    const { data } = await supabase
      ...
      .select('user_id, clerk_user_id, role')
      .eq('household_id', householdId)
    if (!data) return []
    
    const members: HouseholdMember[] = []
    for (const m of data) {
      const uid = m.user_id || m.clerk_user_id
      const name = await getUserDisplayName(uid)
      members.push({ user_id: uid, role: m.role, name })
    }
    return members
  } catch { return [] }
}
