...
export async function saveUserData(userId: string, data: any): Promise<boolean> {
  try {
    const { error } = await supabase
      .from('user_data')
      .upsert({ clerk_user_id: userId, user_id: userId, app_state: data, updated_at: new Date().toISOString() }, { onConflict: 'clerk_user_id' })
    if (error) {
      const { error: err2 } = await supabase
        .from('user_data')
        .upsert({ clerk_user_id: userId, user_id: userId, app_state: data, updated_at: new Date().toISOString() }, { onConflict: 'user_id' })
...
export async function createHousehold(userId: string, name: string): Promise<{ id: string; shareCode: string } | null> {
  try {
    ...
    if (error || !data) return null
    await supabase.from('household_members').insert({ household_id: data.id, clerk_user_id: userId, user_id: userId, role: 'owner' })
...
export async function joinHousehold(userId: string, shareCode: string): Promise<{ id: string; name: string } | null> {
  try {
    ...
    const { data: existing } = await supabase
      .from('household_members').select('id').eq('household_id', household.id)
      .or(`user_id.eq.${userId},clerk_user_id.eq.${userId}`).single()
    if (!existing) {
...
export async function getMyHouseholds(userId: string): Promise<HouseholdInfo[]> {
  try {
    ...
    for (const m of memberships) {
      const { data: hh } = await supabase.from('households').select('id, name, share_code').eq('id', m.household_id).single()
      if (!hh) continue
      const { count } = await supabase.from('household_members').select('id', { count: 'exact', head: true }).eq('household_id', hh.id)
...
export async function leaveHousehold(userId: string, householdId: string): Promise<boolean> {
  try {
    const { error } = await supabase.from('household_members').delete().eq('household_id', householdId)
      .or(`user_id.eq.${userId},clerk_user_id.eq.${userId}`)
    return !error
  } catch { return false }
}
...
export async function renameHousehold(householdId: string, newName: string): Promise<boolean> {
  try {
    const { error } = await supabase.from('households').update({ name: newName }).eq('id', householdId)
    return !error
  } catch { return false }
}
...
export async function getUserDisplayName(userId: string): Promise<string> {
  try {
    const { data } = await supabase.from('household_members').select('display_name')
      .or(`user_id.eq.${userId},clerk_user_id.eq.${userId}`).single()
...
export async function getActivityLog(householdId: string, limit: number = 50): Promise<ActivityEntry[]> {
  try {
    const { data } = await supabase.from('activity_log').select('*')
      .eq('household_id', householdId).order('created_at', { ascending: false }).limit(limit)
    return data || []
  } catch { return [] }
}
