...
function shopSearchUrl(itemText: string): string {
  // Google Shopping searches across Amazon, Walmart, Target, Best Buy, etc.
  return `https://www.google.com/search?tbm=shop&q=${encodeURIComponent(itemText)}`
}
    ...
    return (
      <div className="min-h-[100dvh] max-w-lg mx-auto bg-[#FAF9F6]">
        ...
        <div className="p-4 space-y-3">
                        ...
                        <>
                          ...
                          {item.added_by && (
                            <span className="text-xs text-stone-500">
                              {nameMap[item.added_by] ? `Added by ${nameMap[item.added_by]}` : ''}
          ...
          {/* Household actions */}
          <div className="bg-white rounded-xl border border-stone-100 p-4 mt-4 shadow-sm space-y-3">
            ...
            {renamingList ? (
              <div className="flex gap-2">
                ...
                <button onClick={() => setRenamingList(false)} className="px-2 py-2 text-xs text-stone-500">✕</button>
              </div>
            ) : (
              <button onClick={() => { setRenamingList(true); setRenameText(activeHH.name) }}
                className="flex items-center gap-1.5 text-sm text-stone-700 hover:text-amber-600">
                <Pencil className="w-3 h-3" /> Rename list
              </button>
            )}

            <p className="text-sm text-stone-700">Share code: <span className="font-mono font-bold text-stone-800">{activeHH.shareCode}</span></p>
            <p className="text-sm text-stone-600">{activeHH.memberCount} member{activeHH.memberCount > 1 ? 's' : ''} connected</p>

            {/* Members list */}
            {members.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-1.5">Members</p>
                <div className="space-y-1">
                  {members.map(m => (
                    <div key={m.user_id} className="flex items-center gap-2 text-sm">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${m.role === 'owner' ? 'bg-amber-100 text-amber-700' : 'bg-stone-100 text-stone-600'}`}>
                        {(m.name || '?').charAt(0).toUpperCase()}
                      </div>
                      <span className="text-stone-800 font-medium">{m.name || 'Unknown'}</span>
                      {m.role === 'owner' && (
                        <span className="inline-flex items-center gap-0.5 text-xs text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded-full">
            ...
            <button onClick={async () => {
              ...
              <History className="w-3.5 h-3.5" /> {showLog ? 'Hide activity log' : 'View activity log'}
            </button>

            {showLog && (
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {activityLog.length === 0 ? (
                  <p className="text-xs text-stone-500 italic">No activity yet</p>
                ) : activityLog.map(entry => (
                  <div key={entry.id} className="flex items-start gap-2 text-xs text-stone-600 py-1 border-b border-stone-50">
                    <Clock className="w-3 h-3 flex-none mt-0.5 text-stone-400" />
                    <div>
                      <span className="font-medium text-stone-700">{nameMap[entry.user_id] || 'Someone'}</span>
                      {' '}{entry.action}{' '}
                      {entry.item_text && <span className="text-stone-800">"{entry.item_text}"</span>}
                      <p className="text-stone-400 text-[10px]">{new Date(entry.created_at).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
              </div>
