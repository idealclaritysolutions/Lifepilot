...
import { ArrowLeft, Plus, Users, Copy, Check, Trash2, ShoppingCart, LogOut, Loader2, ChevronRight, StickyNote, Link as LinkIcon, Pencil, ExternalLink, History, Clock, Crown, MoreHorizontal } from 'lucide-react'
...
export function SharedListView({ onClose, state, isTab }: Props) {
  const { user } = useAuth()
  ...
  if (!user || !isPremium) {
    return (
      <div className={`${isTab ? '' : 'min-h-[100dvh]'} max-w-lg mx-auto bg-[#FAF9F6]`}>
        {!isTab && (
                ...
                <div key={item.id} className="bg-white rounded-xl border border-stone-100 px-4 py-3 shadow-sm">
                      ...
                      {!hasLink && !item.checked && (() => {
                        ...
                        return (
                          <a href={smart.url} target="_blank" rel="noopener noreferrer"
                            className="p-1.5 rounded-lg text-emerald-500 hover:bg-emerald-50" title={smart.label}>
                            <ExternalLink className="w-4 h-4" />
                          </a>
                        )
                      })()}
                      {/* Three-dot menu for edit/note/delete */}
                      <button onClick={() => setItemMenu(itemMenu === item.id ? null : item.id!)}
                  ...
                  {itemMenu === item.id && (
                    <div className="flex gap-1 ml-10 mt-1.5">
                      <button onClick={() => { setEditingItem(item.id!); setEditText(item.text); setItemMenu(null) }}
                        className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-stone-50 text-xs text-stone-600 hover:bg-stone-100">
                        <Pencil className="w-3 h-3" /> Edit
                      </button>
                      <button onClick={() => { setEditingNote(editingNote === item.id ? null : item.id!); setNoteText(item.notes || ''); setItemMenu(null) }}
                        className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-stone-50 text-xs text-stone-600 hover:bg-stone-100">
                        <StickyNote className="w-3 h-3" /> Note
                      </button>
                      <button onClick={() => { handleRemove(item.id!); setItemMenu(null) }}
                        className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-red-50 text-xs text-red-500 hover:bg-red-100">
    ...
    <div className={`${isTab ? '' : 'min-h-[100dvh]'} max-w-lg mx-auto bg-[#FAF9F6]`}>
      {!isTab ? (
        ...
        <header className="px-4 pt-4 pb-2">
          <h1 className="text-lg font-bold text-stone-800" style={{ fontFamily: "'Georgia', serif" }}>Shared Lists</h1>
        </header>
      )}
